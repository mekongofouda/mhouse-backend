...
export class PostEntity extends TimestampEntity {
  ...
  @OneToMany((type) => Share, (share) => share.post, { eager: true })
  shares: Share[];

  @OneToMany((type) => Like, (like) => like.post, { eager: true })
  likes: Like[];

  @OneToMany((type) => Offer, (offer) => offer.post, { eager: true })
  offers: Offer[];

  @OneToMany((type) => Sponsor, (sponsor) => sponsor.post, { eager: true })
  sponsors: Sponsor[];
