...
export class SponsorService extends Utils {
  ...
  async sponsor(sponsorDto: SponsorDto, account: any): Promise<Sponsor> {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.SPONSOR) == false
      ) {
  ...
  async listSponsor(listSponsorDto: ListSponsorDto, account: any) {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.LIST_SPONSOR) ==
        false
      ) {
        throw new UnauthorizedException();
      }
    ...
    if (listSponsorDto.refAccount != undefined) {
      const userAccount = await this.accountRepository.findOneBy({
        refAccount: listSponsorDto.refAccount,
      });
      if (userAccount == null) {
        throw new HttpException('Account not found', HttpStatus.NOT_FOUND);
      }
    ...
    if (listSponsorDto.refPost != undefined) {
      const post = await this.postRepository.findOneBy({
        refPost: listSponsorDto.refPost,
      });
      if (post == null) {
        throw new HttpException('Post not found', HttpStatus.NOT_FOUND);
      }
    ...
    listSponsors.filter((sponsor) => {
      if (listSponsorDto.createdAt != undefined) {
        if (
          sponsor.createdAt.toDateString() ==
          listSponsorDto.createdAt.toDateString()
        ) {
      ...
      if (listSponsorDto.updatedAt != undefined) {
        if (
          sponsor.updatedAt.toDateString() ==
          listSponsorDto.updatedAt.toDateString()
        ) {
          if (!sponsors.includes(sponsor)) {
            sponsors.push(sponsor);
          }
  ...
  async showSponsorDetail(refSponsor: string, account: AccountEntity) {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.SHOW_SPONSOR) ==
        false
      ) {
        throw new UnauthorizedException();
      }
