...
export class ShareService extends Utils {
  ...
  async share(shareDto: ShareDto, account: any): Promise<Share> {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.SHARE) == false
      ) {
  ...
  async listShare(listShareDto: ListShareDto, account: any): Promise<Share[]> {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.LIST_SHARE) ==
        false
      ) {
        throw new UnauthorizedException();
      }
    ...
    if (listShareDto.refAccount != undefined) {
      const userAccount = await this.accountRepository.findOneBy({
        refAccount: listShareDto.refAccount,
      });
      if (userAccount == null) {
        throw new HttpException('Account not found', HttpStatus.NOT_FOUND);
      }
    ...
    listShares.filter((share) => {
      if (listShareDto.createdAt != undefined) {
        if (
          share.createdAt.toDateString() ==
          listShareDto.createdAt.toDateString()
        ) {
      ...
      if (listShareDto.updatedAt != undefined) {
        if (
          share.updatedAt.toDateString() ==
          listShareDto.updatedAt.toDateString()
        ) {
          if (!shares.includes(share)) {
            shares.push(share);
          }
  ...
  async showShareDetail(refShare: string, account: AccountEntity) {
    ...
    if (userAccount != null) {
      if (
        this.IsAuthorised(userAccount, FunctionPrivilegeEnum.SHOW_SHARE) ==
        false
      ) {
        throw new UnauthorizedException();
      }
